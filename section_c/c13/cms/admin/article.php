...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {                 // If form submitted
    ...
    if ($_FILES['image']['error'] !== 0) {
      die('Erro no upload: Código ' . $_FILES['image']['error']);
  }
    ...
    if ($temp and $_FILES['image']['error'] === 0) {                   // If file uploaded and no error
        ...
        if ($errors['image_file'] === '' and $errors['image_alt'] === '') { // If valid
            ...
            $test_filename = create_filename($_FILES['image']['name'], $uploads);
            if (!$test_filename) {
              die('Erro ao gerar nome do arquivo.');
            }
    ...
    if ($invalid) {                                              // If invalid
        ...
        try {                                                    // Try to insert data
            ...
            if ($destination) {                                  // If have valid image
                ...
                $imagick = new \Imagick();
                $imagick->readImageBlob(file_get_contents($temp));
